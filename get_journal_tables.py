    ...
    def update_MLD_references(self):
        ...
        with open(join(self.base_directory, 'batch_update_mysql_references.txt'), 'w') as file:
            for self.query in self.update_reference:
                ...
                file.write(r' ) VALUES ( ')
                file.write('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW()'%(1, self.ads_to_mld_reference_interpreter[self.query], po['abstract'], ', '.join(po['authors']), po['Title'], po['journal_name'], po['publication_year'].split('/')[0], po['start_page'], po['publication_year'].split('/')[1], po['start_page'], po['url'], ': '.join(po['keywords']), '', '', '', '', '', '', 'article', '', self.journal_id_converter_inverted[po['journalID']], 1))                if 'number' in po: file.write(','+po['number'])
