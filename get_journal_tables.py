...
class Journal_tables():
    ...
    def set_coord_details(self, standard_name, weight, key, key2, update_status, query, save=True, ra='', dec=''):
        if (save and self.lens_objects[standard_name]['Standard RA'][0]['value']) or (not save and ra):
            ...
            rhour = ('0' if int(rhour) <= 9 else '') + str(int(rhour))
            rmn = ('0' if int(rmn) <= 9 else '') + str(int(rmn))
            rsec = ('0' if int(rsec) <= 9 else '') + str(rsec)
            
            ddeg = ddec_sign + ('0' if int(ddeg) <= 9 else '') + str(int(ddeg))
            dmn = ('0' if int(dmn) <= 9 else '') + str(int(dmn))
            dsec = ('0' if int(dsec) <= 9 else '') + str(dsec)
    ...
    def load_silo_eboss(self):
        ...
        for candidate in hdu['DETECTIONS'].data:
            ...
            self.lens_objects[standard_name]['Discovery'].append({'value': 'SILO', 'tracer': {'bibcode':'MNRAStmp(2021)303T', 'update status': 'in SILO', 'weight':5}})
            if candidate['FIRST_DETECTION_FROM'] == '': self.lens_objects[standard_name]['Detected by'] = {'value': 'SILO', 'tracer': {'bibcode':'MNRAStmp(2021)303T', 'update status': 'in SILO', 'weight':5}}
